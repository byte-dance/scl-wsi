...
from Classify2_AssemblyLine_Primary_L00 import CellsClassificationTwoL00
...
def get(bucket, json_data, path, name):
    ...
    pool_get = ThreadPoolExecutor(max_workers=10)
    for obj in oss2.ObjectIterator(bucket, prefix=json_data):
        ...
        if "L00" in file_path or "L02A" in file_path or ".txt" in file_name or "Data" in file_path:
    ...
    def run(self):
        while True:
            ...
            if not self.sample_queue.empty():
                ...
                start = time.time()
                try:
                    if os.path.exists(os.path.join(local_sample, 'Blocks/L02A')):
                        yolo = YOLO()
                        ...
                        yolo = YOLOL00()
                        yolo.get_positive_num(local_sample)
                        CellsClassificationJunkL00(local_sample, preload=False).process()
                        CellsClassificationTwoL00(local_sample, preload=False).process()
                        # CellsClassificationMulit(local_sample, preload=False).process()
                except Exception as e:
                ...
                with open("exist_samples_9N.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    # excel路径
    excel_path = 'all_file/9N.xls'
    # 读取excel
    e = pd.read_excel(excel_path, sheet_name='Sheet1')
    exist_samples = open("exist_samples_9N.txt").read().strip().split()
    sample_names = []
