...
# 数据目录
class CellsClassificationTwo(object):
    ...
    def process(self):
        ...
        for batch in test_lump_data:
            ...
            for v in self.swinv2_lump_model(batch)[1]:
                svm_lump_feature.append(v.data.cpu())

        for i, cell in enumerate(lump_cells):
            cell.append(float(lump_positiveScores[i]))
            cell.append(float(lump_negativeScores[i]))
            cell.append(svm_lump_feature[i])

        res_lump = sorted(lump_cells, key=lambda x: x[8], reverse=True)
        ...
        for batch in test_data:
            ...
            for v in self.swinv2_model(batch)[1]:
                svm_feature.append(v.data.cpu())
        ...
        for i, cell in enumerate(nolump_cells):
            ...
            cell.append(svm_feature[i])

        res_cell = sorted(nolump_cells, key=lambda x: x[8], reverse=True)

        if len(cells) >= 16:
            if len(res_cell) >= 8:
                if len(res_lump) >= 8:
                    res_cells = res_cell[0:8] + res_lump[0:8]
                else:
                    res_cells = res_cell[0:(16 - len(res_lump))] + res_lump
            else:
                res_cells = res_cell + res_lump[0:(16 - len(res_cell))]

            for p in res_cells[0:16]:
                feature_vector = p[10]
                with h5py.File('./feature0326/feature_vector_L02A_9N.h5', 'a') as hf:
                    Feature_Vector_Counter = len(hf)
                    hf.create_dataset(str(Feature_Vector_Counter + 1), data=feature_vector)
                    hf.close()
            with open("./feature0326/exist_samples_feature_L02A_9N.txt", "a") as f:
                f.write(os.path.basename(self.datapath))
                f.write("\n")

        p_cells_result = list(filter(lambda x: x[8] > x[9], nolump_cells))
        print("P_cells/cells: ", len(p_cells_result), '/', len(nolump_cells))

        # with open('test_epoch_75_ckpt_epoch_95_0226(P6).txt', 'a+') as f:
