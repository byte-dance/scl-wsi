...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return tmp_inp, self.labels[index]
...
#       创建YOLO类
# ------------------------------------- #
class YOLOL00(object):
    _file_path = os.path.dirname(__file__)
    _defaults = {
        "model_path": os.path.join(_file_path, 'logs/last_epoch_weights_v7.pth'),  # yolov7
                ...
                if new_boxes_all[i] is not None:
                    with open(samplepath + "/positive_Cells.txt", 'a') as f:
                        for k, cell in enumerate(new_boxes_all[i]):
                            width = int(cell[3] - cell[1])
                            height = int(cell[2] - cell[0])
                            sz = max(width, height) if max(width, height) > 256 else 256
                            f.write(new_labels[i][1] + ',' + str(new_labels[i][2]) + ',' + ',' + str(sz) + ',')
                            f.write(str(cell[0]) + ',' + str(cell[1]) + ',' + str(cell[2]) + ',' + str(cell[3]) + ',' + str(confs[i][k]) +  '\n')
    ...
    def get_imageinfo(self, samplepath):
        image_file = os.path.join(samplepath, "Blocks/L00")
        # block_json_Cytoptic = os.path.join(samplepath, "Data/BlocksJson.Cytoptic")
        block_json = os.path.join(samplepath, "Data/BlocksJson.json")
        # shutil.copy(block_json_Cytoptic,block_json)
        with open(samplepath + "/positive_Cells.txt", 'a') as f:
